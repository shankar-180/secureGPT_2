<script lang="ts">
	...
	let domain = "";
	let app_name = PUBLIC_APP_NAME;
	onMount(() => {
		domain = window.location.hostname;

		if (domain.includes("securegpt.accure.ai")) {
			app_name = "SecureGPT";
		} else if (domain.includes("legalgpt.accure.ai")) {
			app_name = "LegalGPT";
		}
	});
</script>
...
<Modal on:close>
	...
	>
		...
		<p class="rounded-xl border bg-white/80 p-2 text-base text-gray-800">
			You have reached the guest message limit, please Sign In with your username and password to
			continue.
		</p>
			...
			{#if $page.data.loginRequired}
				...
				>
					...
					{#if PUBLIC_APP_NAME === "HuggingChat"}
						with <LogoHuggingFaceBorderless classNames="text-xl mr-1 ml-1.5" /> SecureGPT
					{/if}
				</button>
			{:else}
