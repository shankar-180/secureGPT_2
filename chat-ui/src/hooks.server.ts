...
export const handle: Handle = async ({ event, resolve }) => {
	...
	if (user) {
		const userSettings = await collections.userSettings.findOne({ "email": user.email, "org": user.org, "group": user.group });
		const assistant = await collections.assistants.findOne({ "email": user.email, "org": user.org, "group": user.group, "enabled": true, "status": "active" });
		...
		sessionStore.update((session) => {
			...
			if (userSettings != null) {
				session.contextMode = userSettings.contextMode;
			}
			if (assistant != null) {
	...
	if (event.request.method === "POST" && nativeFormContentTypes.includes(requestContentType)) {
		...
		// console.log(new URL(referer).origin);
		// if (!referer.includes("localhost") && !validOrigins.includes(new URL(referer).origin)) {
