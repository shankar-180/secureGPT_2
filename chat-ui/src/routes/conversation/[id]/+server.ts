...
export async function POST({ request, fetch, locals, params, getClientAddress }) {
	...
	if(email){
		...
		//throw error(401, "Unauthorized");
		console.log("User is not logged in and sesssion does not exist.");
	}
	...
	const stream = new ReadableStream({
		async start(controller) {
			...
			console.log("random URL:"+ randomEndpoint.authorization);
			const tokenStream = textGenerationStream(
				{
					...
					accessToken: randomEndpoint.authorization, //randomEndpoint.host === "sagemaker" ? undefined : HF_ACCESS_TOKEN,
