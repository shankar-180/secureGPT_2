...
## Changes made in the open source version
Modified for OAuth:
	modified:   src/app.html
	modified:   src/hooks.server.ts : !referer.includes("localhost") 
	modified:   src/lib/components/LoginModal.svelte: Message changed from huggingface to Auth0
	modified:   src/lib/components/ModelCardMetadata.svelte: Changed huggingface URL to accure.ai
	modified:   src/lib/components/NavMenu.svelte: huggingface to Accure. Also added Document Upload menu
	modified:   src/lib/components/chat/ChatIntroduction.svelte : Huggingface to Accure (Making the community's best AI chat models available to everyone.)
	modified:   src/lib/components/chat/ChatWindow.svelte: Huggingface to Accure
	modified:   src/lib/server/auth.ts : No significant change. Only logs to check the flow of user data.
	modified:   src/lib/server/database.ts : Added Documents as a type so that uploaded docs are saved in MongoDB documents collection
	modified:   src/routes/+layout.svelte: No significant change
	modified:   src/routes/login/callback/+page.server.ts : Only console log
	modified:   src/routes/login/callback/updateUser.ts : This is the main file where we added two columns role, and org to save in Mongodb.(lines: 16, 17, 24, 25, 33, 34, 63, 64)
  modified:   src/routes/login/callback/updateUsers.ts: Line 47, added email in session store
Modified for sending org with query:
  modified:   src/routes/conversation/[id]/+server.ts: Line 232 to pass org parameter
  modified:   src/lib/server/generateFromDefaultEndpoint.ts : Line 62 to pass org parameter
  
Added new for file upload:
src/lib/sessionStore.js : This is where email is saved in the session store
	src/lib/types/Document.ts : This is a new type for saving data in MongoDB
	src/routes/docs/ : Main route for all document related functions
  src/routes/docs/+page.svelte : Main doc page
  src/routes/docs//+server.js : 
  src/routes/docs/+page.server.js
  src/routes/docs/upload/+server.js
  src/routes/docs/upload/db/+server.js

Modified for session management
Modified: hooks.server.js: Added in the session store the user details.
...
Package installed:
  ...
  npm install mime
  npm install archiver extract-zip yauzl

# ------- Deployment Instructions --------------------
  # Build for production
    npm run build
    npm install -g pm2
    #Run the pm2 server at port 3000
    pm2 start --name securegpt-ui build/index.js
    #Configure the Nginx server as follows:
   # --------------------s
    server {
    listen      8443;
    server_name securegpt.accure.ai;
